...
fn import(input: LocatedSpan) -> IResult<Token> {
    ...
    map_once(
        ...
        move |(tag, args, from, lquote, filename, _, block)| {
            ...
            if state.borrow().cached_imports.contains_key(&path) {
                // Already imported?
                let tokens_is_empty = state
                    .borrow()
                    .cached_imports
                    .get(&path)
                    .unwrap()
                ...
                let mut imported_file = ImportedFile {
                    tokens: Arc::new(vec![]),
                    path: path.clone(),
                    full_span: None,
                };
                ...
                state
                    .borrow_mut()
                    .cached_imports
                    .insert(imported_file.path.clone(), imported_file.clone());

                let current_file = state.borrow().current_file.clone();

                state.borrow_mut().add_file(&imported_file.path).unwrap();
                let tokens = parse_with_state(state.clone());
                imported_file.tokens = Arc::new(tokens);
                imported_file.full_span = Some(state.borrow().current_file.span);
                ...
                state
                    .borrow_mut()
                    .cached_imports
                    .insert(imported_file.path.clone(), imported_file);
            };

            let imported_file = state.borrow().cached_imports.get(&path).unwrap().clone();
...
pub fn parse(
    ...
    let tree = Arc::new(ParseTree::new(state.code_map, tokens));
    ...
    fn parse_import() {
        ...
        match token {
            Token::Import { imported_file, .. } => {
                ...
                assert_eq!(imported_file.tokens.len(), 2);
                match imported_file.tokens.first() {
