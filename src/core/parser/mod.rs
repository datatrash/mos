        ...
        match parser(input) {
            ...
            Err(nom::Err::Error(_)) | Err(nom::Err::Failure(_)) => {
                ...
                if message.is_empty() {
                    ...
                    i.extra.shared_state.borrow_mut().report_error(err);
                }
                Ok((i, None))
            }
            Err(err) => Err(err),
    ...
    while count > 0 {
        ...
        if new_input.fragment().starts_with("/*") {
            ...
            if tag.is_none() {
                ...
                ni.extra.shared_state.borrow_mut().ignore_next_error();
                return rest(ni);
            }
            new_input = ni;
        }
...
fn braces(input: LocatedSpan) -> IResult<Token> {
    ...
    map_once(block, move |block| {
        let scope = state.shared_state.borrow_mut().new_anonymous_scope();
...
fn error(input: LocatedSpan) -> IResult<Token> {
    map_once(
        ...
        move |(input, char)| {
            ...
            input.data.extra.shared_state.borrow_mut().report_error(err);
...
fn loop_(input: LocatedSpan) -> IResult<Token> {
    ...
    map_once(
        ...
        |(tag, expr, block)| {
            let loop_scope = Box::new(state.shared_state.borrow_mut().new_anonymous_scope());
...
fn import(input: LocatedSpan) -> IResult<Token> {
    ...
    map_once(
        ...
        move |(tag, args, from, lquote, filename, _, block)| {
            let import_scope = state.shared_state.borrow_mut().new_anonymous_scope();
...
pub fn parse(
    ...
    while !files_to_import.is_empty() {
        ...
        let file = state.borrow_mut().add_file(to_import.clone());
        if file.is_err() {
    ...
    let state = Arc::try_unwrap(state).ok().unwrap().into_inner();
