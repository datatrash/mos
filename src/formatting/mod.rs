use crate::core::parser::{
    AddressingMode, ArgItem, Block, Expression, ExpressionFactor, Located, ParseTree, Token, Trivia,
};
...
impl<'a> CodeFormatter<'a> {
    ...
    fn format_located_token<'f: 'a>(&mut self, lt: &'f Located<Token>) -> String {
        format!(
            "{}{}",
            self.format_located_trivia(lt),
            self.format_token(&lt.data)
        )
    }
        ...
        let result = match &token {
            Token::Align { tag, value } => {
                ...
                let value = space_prefix_if(
                    format!(
                        "{}{}",
                        self.format_located_trivia(value),
            ...
            Token::Instruction(i) => {
                ...
                let mnemonic = match self.prev_non_trivia_token() {
                    Some(Token::Instruction(_)) => {
                        ...
                        } else if self.format_located_trivia(&i.mnemonic).is_empty() {
                            format!(" {}", mnemonic)
                        } else {
                            mnemonic
                        }
                ...
                let operand = match &i.operand {
                    Some(o) => {
                        ...
                        let expr = format!(
                            "{}{}",
                            self.format_located_trivia(&o.expr),
            ...
            match f.options.braces.position {
                BracePosition::SameLine => {
                    ...
                    let lparen = format!(
                        ...
                        space_prefix(
                            f.format_located_trivia(&block.lparen)
                                .trim_start()
                                .to_string()
                        ),
                        "\n",
                    ...
                    let rparen = format!(
                        ...
                        f.format_located_trivia(&block.rparen)
                    );
    ...
    fn format_located<T: Display>(&self, lt: &Located<T>) -> String {
        format!("{}{}", self.format_located_trivia(lt), lt.data)
    }

    fn format_optional_located<T: Display>(&self, lt: &Option<Located<T>>) -> String {
        lt.as_ref()
            .map(|lt| format!("{}{}", self.format_located_trivia(&lt), lt.data))
            .unwrap_or_else(|| "".to_string())
    }
    ...
    fn format_located_trivia<T>(&self, lt: &Located<T>) -> String {
        lt.trivia
            .as_ref()
    ...
    fn format_trivia(&self, trivia: &[Trivia]) -> String {
        if self.options.whitespace.trim {
            ...
            trivia.iter().map(|item| format!("{}", item)).join("")
        }
    }
    ...
    fn format_expression(&mut self, expr: &'a Expression) -> String {
        match expr {
            Expression::BinaryExpression(be) => {
                ...
                let lhs = format!(
                    "{}{}",
                    self.format_located_trivia(&be.lhs),
                    self.format_expression(&be.lhs.data)
                );
                let rhs = format!(
                    "{}{}",
                    self.format_located_trivia(&be.rhs),
            ...
            Expression::Factor {
                ...
                let factor = format!(
                    "{}{}",
                    self.format_located_trivia(factor),
    ...
    fn format_expression_factor(&mut self, factor: &'a ExpressionFactor) -> String {
        match factor {
            ...
            ExpressionFactor::ExprParens {
                ...
                let inner = format!(
                    "{}{}",
                    self.format_located_trivia(inner),
...
pub fn format(ast: &ParseTree, options: FormattingOptions) -> String {
...
mod tests {
    ...
    /*
        #[test]
        fn default_formatting_keeps_comments() -> MosResult<()> {
    ...
    */
    ...
    #[test]
    fn passthrough_formatting_should_result_in_unformatted_original() -> MosResult<()> {
        let source = include_str!("../../test/cli/format/valid-unformatted.asm");
    ...
    #[test]
    fn passthrough_formatting_should_result_in_formatted_original() -> MosResult<()> {
